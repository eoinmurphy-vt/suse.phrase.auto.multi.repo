...
O recurso `+Schedule+` define janelas de tempo durante as quais o {product_name} aceita alterações no `+Clusters+`.

Para obter mais informações sobre como usar o recurso `+Schedule+`, consulte xref:experimental-features/scheduling.adoc[Agendamento].
