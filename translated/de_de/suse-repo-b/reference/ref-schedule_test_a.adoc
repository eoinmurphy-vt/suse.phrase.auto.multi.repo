...
Die Ressource `+Schedule+` definiert Zeitfenster, in denen {product_name} Änderungen an `+Clusters+` akzeptiert.

Weitere Informationen über die Verwendung der Ressource `+Schedule+` finden Sie unter xref:experimental-features/scheduling.adoc[Zeitplanung].
